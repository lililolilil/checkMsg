...
setCookieValue= function(val_messageDir){
    ...
    //modal table 에 값을 넣고 
    var confirmbody = '<p> 전체 페이지의 기본 경로와 입력하신 기본 경로가 다릅니다. </p> '
	+'<p> 현재 입력하신 경로로 수정 하시려면 [기본경로수정] 버튼을 눌러주세요.</p>'
...
var getMessageFile = function() {
    ...
    $input_Dir = $("#s_messagefileDir"); 

    if(!util.validateData($input_Dir)){
	return false; 
    } 

    var val_messageDir = $("#s_messagefileDir").val(); 
